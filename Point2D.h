...
/** @class Point2D
	@brief The Point2D class creates objects representing an ordered pair (x,y).  The Point2D class stores its values as int. Functions are included to construct and print the objects.  The virtual keyword is implemented to allow pointers of base or derived objects to call the correct form of functions
 */
class Point2D {
public:
    ...
    /** Prints the Point2D object. Virtual keyword is added to permit the Point2D::print() function to be differentiated from the ColorPoint2D and WeightedPoint2D versions
     */
    virtual void print() const {
        std::cout << "(" << x << "," << y << ")";
    }
    
    /** Deletes the Point2D object in the heap. Virtual keyword is added to prevent memory leaks as extra info contained in ColorPoint2D and WeightedPoint2D objects will be deleted also
     */
    virtual ~Point2D() {}
    
    /** Returns int value denoting type of Point2D object user selected. Virtual keyword is added to permit the Point2D::rank() function to be differentiated from the ColorPoint2D and WeightedPoint2D versions
     */
    virtual int rank() {
        return 1;
    }
...
/**	Operator<< is overloaded to output coordinates when an ostream object is located to the left and a Point2D object is located to the right
